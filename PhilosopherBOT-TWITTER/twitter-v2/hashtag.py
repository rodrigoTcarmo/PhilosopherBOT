    ...
    def on_status(self, status):
        ...
        for tweet in tweepy.Cursor(api.search, search, since='2019-06-03').items(numero):
            ...
            try:
                ...
                log.info("RETIRANDO O USER :" + retirando_user)
                tratando_status = re.sub(r'https://.*[\r\n]*', '', retirando_user)
                log.info("TRATANDO STATUS :" + tratando_status)
                ...
                log.info("TIRANDO A HASHTAG:" + pegando_status2)
                # ------------------------------------------------------------------------------
                if pegando_status2 == '':
                ...
                # ----------------------------------------------------------------------
                # ----------------------- WRITE TEXT IN IMAGE ---------------------
                escrever.text(xy=(43, 110), text=textwrap.fill(str(pegando_status2), 28), fill=(255, 255, 255),
                              font=fonte)

                # -------------------------------------------------------------------------------
                # ----------------------- CHOICE PHILOSOPHER PLACE AND POST ---------------------
            ...
            # ----------------------- ERRO EXECPT ---------------------
                ...
                log.info(e.reason)
                log.info("Não foi possivel coletar ID do usuário.")
                log.info("\n==========================")
